...
2. JSON의 title 필드는 반드시 다음 규칙을 따른다.
   ...
   - 핵심 키워드를 포함해서 10~30자 내의 자연스러운 한국어 제목으로 작성한다.
   - 클릭을 유도하도록 자극적인 제목을 사용하되 불법적이지 않아야한다.
...
6. 자연스러운문맥으로 글자 수 요구를 충실히 반영한다.

7. 모델 관련 메타 설명, 사족, 해설, 주석 등은 일절 포함하지 않는다.
...
def _build_user_prompt_from_records(title, ages, contry_type, sex, type, length) -> str:
    ...
    # 내용 밀도 강화
    lines.append(
        "각 섹션에서는 가능하면 다음 요소 중 2개 이상을 포함해서 내용을 풍부하게 만들어줘:"
    ...
    lines.append(
        ...
        "예: {{국민연금공단 로고 사진}}, {{서울 도심 전경 사진}} 처럼 한 줄에 단독으로 넣어줘. "

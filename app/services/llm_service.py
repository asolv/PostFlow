...
SYSTEM_PROMPT = """
너의 역할은 ‘콘텐츠 생성기’이다.
너는 각각의 카테고리의 최고의 전문가이며 관련 모든법과 최신 유행을 잘 알고있다.
...
3. 컨텐츠는 자극적이되 불법적이지 않으며 이해하기 쉽고 가독성이 좋아야한다.
    ...
    - 개인적인 상식적, 합리적 견해가 포함되도록하나 불법적이지 않아야한다.

4. 섹션별로 user_prompt가 요구한 형식대로 사진 title 자리 표시를 한줄에 단독으로 넣는다.
    - 2단어 이하의 영어로 입력한다.
    - 예: `[[Hair]]`
...
def _build_user_prompt_from_records(title, ages, contry_type, sex, type) -> str:
    ...
    # 이미지 placeholder
    lines.append(
        "맥락과 맥락 사이, 또는 큰 섹션 사이에 적절한 사진의 title을 [[]] 형태로 영어로 넣어줘. "
        "이 [[]]안에 있는 타이틀은 나중에 pexels에서 이미지검색하기 쉽도록 쉬운단어로 1~2단어로 해줘"
        "만약 title이 한국인물일경우에는 한글로 명시해주고 그 외에는 영어로 명시해줘"
        "예: [[..]] 처럼 한 줄에 단독으로 넣어줘. "
    ...
    lines.append(
        ...
        "\n- summary: 전체 내용을 아우르는 한가지 키워드를 [[keyword]] 형태로 영어로 적어놓고 이후에 전체 내용을 4~6문장 정도로 요약하되, content의 핵심 포인트가 빠지지 않게 써."
        "\n- content: 위에서 요구한 구조와 길이 조건을 모두 만족하는 긴 본문을 작성해."
        "\n- tags: 이곳에 content에 작성된 본문의 주요 키워드들을  넣어줘.그냥 string 형태이고 각 tag들의 구분자는 | 로 구분해줘"
...
def generate_rss_feed_by_gpt(keyword, ages, contry_type, sex, type):
    if not keyword:
        return {"items": []}

    user_prompt = _build_user_prompt_from_records(
        keyword, ages, contry_type, sex, type
    )
